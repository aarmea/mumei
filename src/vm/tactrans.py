...
def translate(tac):
  ...
  for inst in tac:
    if isinstance(inst, threeaddr.Label):
      ...
      # destination operand of an instruction.
      appendInst(
        Set(RegOperand(Processor.REG_X3), RegOperand(Processor.REG_FL))
      )

      # Check whether the flags are zero
      appendInst(
        Or(RegOperand(Processor.REG_X3), ShortImmOperand(0))
      )
      ...
      # Invert the zero flag
      appendInst(
        Set(RegOperand(Processor.REG_FL), ShortImmOperand(Processor.FLAG_ZERO))
      )
